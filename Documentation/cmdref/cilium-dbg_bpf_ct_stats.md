<!-- This file was autogenerated via cilium-dbg cmdref, do not edit manually-->

## cilium-dbg bpf ct stats

Connection tracking statistics

### Options

```
  -h, --help   help for stats
```

### Options inherited from parent commands

```
      --config string        Config file (default is $HOME/.cilium.yaml)
  -D, --debug                Enable debug messages
  -H, --host string          URI to server-side API
      --log-driver strings   Logging endpoints to use (example: syslog)
      --log-opt map          Log driver options (example: format=json)
```

### SEE ALSO

* [cilium-dbg bpf ct](cilium-dbg_bpf_ct.md)	 - Connection tracking tables
* [cilium-dbg bpf ct stats flush](cilium-dbg_bpf_ct_stats_flush.md)	 - Flush all connection tracking entries
* [cilium-dbg bpf ct stats list](cilium-dbg_bpf_ct_stats_list.md)	 - List connection tracking statistics

