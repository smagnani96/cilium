...
func (m *Map) doGCForFamily(filter GCFilter, next4, next6 func(GCEvent), ipv6 bool) gcStats {
	...
	if ipv6 {
		...
		stats.dumpError = iterate[CtKey6Global, CtEntry](ctx, m, func(key bpf.MapKey, value bpf.MapValue) bool {
			filterCallback(key, value)
			return true
		})
	} else {
		filterCallback := m.cleanup(filter, natMap, &stats, next4, ipv6)
		stats.dumpError = iterate[CtKey4Global, CtEntry](ctx, m, func(key bpf.MapKey, value bpf.MapValue) bool {
			filterCallback(key, value)
			return true
		})
	}
...
func iterate[KT any, VT any, KP bpf.KeyPointer[KT], VP bpf.ValuePointer[VT]](ctx context.Context, m *Map, filterCallback func(key bpf.MapKey, value bpf.MapValue) bool) error {
...
func (m *Map) DumpEntries(ctx context.Context, cb func(CtKey, *CtEntry) bool) error {
	wrap := func(key bpf.MapKey, value bpf.MapValue) bool {
		return cb(key.(CtKey), value.(*CtEntry))
	}
	if m.mapType.isIPv6() {
