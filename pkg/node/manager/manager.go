...
// GetNodeByIdentity returns a copy of the node registered under identity,
// if any.
func (m *manager) GetNodeByIdentity(identity nodeTypes.Identity) (nodeTypes.Node, bool) {
	m.mutex.RLock()
	entry, ok := m.nodes[identity]
	m.mutex.RUnlock()
	if !ok {
		return nodeTypes.Node{}, false
	}
