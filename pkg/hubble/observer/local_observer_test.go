...
func TestLocalObserverServer_GetFlows(t *testing.T) {
	...
	pp, nm, me := noopParser(t), testutils.NoopNamespaceManager, testutils.NoopCTExporter
...
func TestLocalObserverServer_GetNamespaces(t *testing.T) {
	...
	s, err := NewLocalServer(pp, nsManager, testutils.NoopCTExporter, hivetest.Logger(t), observeroption.WithMaxFlows(container.Capacity1))
	require.NoError(t, err)
...
func Benchmark_TrackNamespaces(b *testing.B) {
	pp := noopParser(b)
	nsManager := namespace.NewManager()
	s, err := NewLocalServer(pp, nsManager, testutils.NoopCTExporter, hivetest.Logger(b), observeroption.WithMaxFlows(container.Capacity1))
	if err != nil {
		b.Fatal(err)
	}
	f := &flowpb.Flow{
