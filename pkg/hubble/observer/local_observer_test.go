...
func TestNewLocalServer(t *testing.T) {
	...
	s, err := NewLocalServer(pp, nm, nil, hivetest.Logger(t))
...
func TestLocalObserverServer_ServerStatus(t *testing.T) {
	pp, nm := noopParser(t), testutils.NoopNamespaceManager
	s, err := NewLocalServer(pp, nm, nil, hivetest.Logger(t), observeroption.WithMaxFlows(container.Capacity1))
	require.NoError(t, err)
...
func TestLocalObserverServer_GetFlows(t *testing.T) {
	...
	pp, nm, me := noopParser(t), testutils.NoopNamespaceManager, testutils.NoopCTStatsExporter
	s, err := NewLocalServer(pp, nm, me, hivetest.Logger(t),
...
func TestLocalObserverServer_GetNamespaces(t *testing.T) {
	...
	s, err := NewLocalServer(pp, nsManager, testutils.NoopCTStatsExporter, hivetest.Logger(t), observeroption.WithMaxFlows(container.Capacity1))
	require.NoError(t, err)
...
func Benchmark_TrackNamespaces(b *testing.B) {
	pp := noopParser(b)
	nsManager := namespace.NewManager()
	s, err := NewLocalServer(pp, nsManager, testutils.NoopCTStatsExporter, hivetest.Logger(b), observeroption.WithMaxFlows(container.Capacity1))
	if err != nil {
		b.Fatal(err)
	}
	f := &flowpb.Flow{
