...
int nodeport_lb6_fragment1_setup(struct __ctx_buff *ctx)
{
	...
	endpoint_v6_add_entry((union v6addr *)BACKEND_IPV6, BACKEND_IFINDEX, 0, 0, 0,
			      (__u8 *)mac_one, (__u8 *)mac_two);
	lb_v6_add_service_with_flags((union v6addr *)FRONTEND_IPV6, FRONTEND_PORT, IPPROTO_TCP,
...
int nodeport_lb6_fragment1_check(struct __ctx_buff *ctx)
{
	...
	struct ct_entry *ct_entry;
	struct ipv6_frag_id expected_frag_id = EXPECTED_FRAG_ID6;
	struct ipv6_ct_tuple expected_ct_tuple = EXPECTED_CT_TUPLE6;
	struct metrics_key metric_key = EXPECTED_METRIC_KEY;
	__u64 metric_count = 3;
	__u64 count = 1;
	__u64 bytes = 0;
	...
	/* Ensure fragment-related metrics are updated accordingly. */
	assert_metrics_count(metric_key, metric_count);
