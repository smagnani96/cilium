...
/**
 * _update_trace_metrics
 * @ctx:	socket buffer
 * @obs_point:	observation point (TRACE_*)
 * @flags:	additional trace flags (CLS_FLAG_*)
 * @line: line of code of the caller
		   ...
		   __u16 line, __u8 file)
{
	...
	bool emit = emit_trace_notify(obs_point, monitor);
	flags = ctx_classify(ctx, emit);
